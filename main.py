...
def load_spritesheet(image_path, frame_width, frame_height, num_frames, flip=False):
    try:
        ...
        print(f"Loaded {image_path}: {sheet.get_width()}x{sheet.get_height()}")
...
while running:
    ...
    if not dragging and current_time - event_timer > EVENT_INTERVAL and special_duration == 0:
        ...
        if active_event == "walk":
            ...
            vfx_state = "meow_vfx_right" if facing_direction == "right" else "meow_vfx_left"
    ...
    if vfx_active:
        ...
        if vfx_frame_counter >= animation_speed:
            vfx_frame += 1
            vfx_frame_counter = 0
            if vfx_frame >= len(animations[vfx_state]):
                vfx_active = False
                vfx_frame = 0

    # Update display
    screen.fill((0, 0, 0))
    if current_state in animations and len(animations[current_state]) > 0:
        screen.blit(animations[current_state][current_frame], pet_rect)
    else:
        print(f"Missing animation: {current_state}")
